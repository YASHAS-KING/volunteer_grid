...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _pickAndUploadAvatar() async {
    ...
    try {
      ...
      if (mounted) _show('Upload Error: $e', Colors.red);
    } finally {
  ...
  //     final url = _supabase.storage.from('avatar').getPublicUrl(path);
  ...
  Future<void> _savePassword() async {
    if (_newPassCtrl.text.length < 8) {
      _show('Password must be at least 8 characters', Colors.red);
      return;
    }
    if (_newPassCtrl.text != _confirmPassCtrl.text) {
      _show('Passwords do not match', Colors.red);
      return;
    }
  ...
  void _show(String msg, Color color) => ScaffoldMessenger.of(
    context,
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Center(
            child: Text(
              _nameCtrl.text.isNotEmpty ? _nameCtrl.text : email,
          ...
          Center(
            child: Container(
              ...
              child: Text(
                role == 'organization' ? 'Organisation' : 'Volunteer',
                style: TextStyle(
                  fontSize: 12,
                  color: primary,
