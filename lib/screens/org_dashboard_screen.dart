...
const _eventTags = [
  'Education', 'Nature', 'Community', 'Outdoor', 'Indoor',
  'Hackathon', 'Health', 'Sports', 'Arts', 'Animal Welfare',
  'Disaster Relief', 'Elderly Care', 'Children', 'Environment',
];
...
class _OrganizationDashboardScreenState extends State<OrganizationDashboardScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final events = await _supabase.from('events').select().eq('org_id', uid).order('created_at', ascending: false);
      if (mounted) {
        setState(() {
          _profile = profile;
        ...
        builder: (ctx, setModal) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text('Post New Event', style: Theme.of(ctx).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                ...
                GestureDetector(
                  onTap: () async {
                    ...
                    if (result != null && result.files.single.path != null) {
                      setModal(() => pickedImagePath = result.files.single.path);
                    }
                  },
                  child: Container(
                    ...
                    child: pickedImagePath != null
                        ...
                        : const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                            ...
                            SizedBox(height: 6),
                            Text('Tap to add event photo (optional)', style: TextStyle(color: Colors.grey, fontSize: 12)),
                          ]),
                  ),
                ),
                const SizedBox(height: 12),
                  ...
                  child: Column(
                    ...
                    children: [
                      if (selectedTags.isNotEmpty)
                        Wrap(
                          ...
                          children: selectedTags.map((t) => Chip(
                            ...
                            onDeleted: () => setModal(() => selectedTags.remove(t)),
                            backgroundColor: (tagColors[t] ?? Colors.deepPurple).withValues(alpha: 0.15),
                      ...
                      DropdownButton<String>(
                        ...
                        hint: const Text('Select tags…', style: TextStyle(fontSize: 13)),
                        value: null,
                        items: _eventTags
                            ...
                            .map((t) => DropdownMenuItem(
                                  value: t,
                                  child: Row(children: [
                                    Icon(tagIcons[t] ?? Icons.label, size: 18,
                            ...
                            .toList(),
                        onChanged: (val) { if (val != null) setModal(() => selectedTags.add(val)); },
                      ),
                    ],
                  ),
                ),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      ...
                      Text(
                        pickedDate == null ? 'Pick Date' : '${pickedDate!.day}/${pickedDate!.month}/${pickedDate!.year}',
                        style: TextStyle(fontSize: 15, color: pickedDate == null ? Colors.grey[600] : Colors.black87),
                      ),
                    ]),
                  ),
                ),
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(fontSize: 15, color: pickedTime == null ? Colors.grey[600] : Colors.black87),
                      ),
                    ]),
                  ),
                ),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 16),
                    decoration: BoxDecoration(
                      border: Border.all(color: pickedLocation != null ? Theme.of(context).colorScheme.primary : Colors.grey.shade400),
                      borderRadius: BorderRadius.circular(12),
                      color: pickedLocation != null ? Theme.of(context).colorScheme.primary.withValues(alpha: 0.05) : null,
                    ),
                    child: Row(children: [
                      Icon(Icons.location_on_outlined,
                          color: pickedLocation != null ? Theme.of(context).colorScheme.primary : Colors.grey),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Text(
                          pickedLocation == null ? 'Pick Location on Map' : pickedLocationLabel,
                          style: TextStyle(
                            fontSize: 14,
                            color: pickedLocation == null ? Colors.grey[600] : Theme.of(context).colorScheme.primary,
                          ),
                  ...
                  child: ElevatedButton(
                    onPressed: posting ? null : () async {
                      if (titleCtrl.text.trim().isEmpty) {
                        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Please enter an event title')));
                        return;
                      }
                      ...
                      try {
                        ...
                        if (pickedDate != null && pickedTime != null) {
                          dateStr = '${pickedDate!.day}/${pickedDate!.month}/${pickedDate!.year} at ${pickedTime!.format(ctx)}';
                        } else if (pickedDate != null) {
                          dateStr = '${pickedDate!.day}/${pickedDate!.month}/${pickedDate!.year}';
                        }
                        ...
                        if (pickedImagePath != null) {
                          ...
                          final path = 'events/$uid/${DateTime.now().millisecondsSinceEpoch}.$ext';
                          await _supabase.storage.from('avatars').uploadBinary(path, bytes, fileOptions: const FileOptions(upsert: true));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Org Dashboard', style: GoogleFonts.playfairDisplay(fontWeight: FontWeight.bold, fontSize: 24, color: primary)),
        actions: [
          IconButton(
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                Stack(alignment: Alignment.bottomRight, children: [
                  CircleAvatar(
                    ...
                    child: avatarUrl == null ? const Icon(Icons.business, size: 40, color: Colors.deepPurple) : null,
                  ),
                  GestureDetector(
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(orgName, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                _statCard(context, 'Events\nPosted', '${_events.length}', Icons.event_available),
                _statCard(context, 'Volunteers\nReached', '${_events.length * 12}', Icons.people_outline),
                _statCard(context, 'Hours\nGenerated', '${_events.length * 20}', Icons.timer_outlined),
              ]),
              const SizedBox(height: 32),

              Text('Posted Events', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),
              ...
              if (_events.isEmpty)
                Container(
                  ...
                  child: const Column(children: [
                    ...
                    SizedBox(height: 12),
                    Text('No events posted yet.', style: TextStyle(color: Colors.grey)),
                    Text('Tap + New Event to get started.', style: TextStyle(color: Colors.grey, fontSize: 12)),
                  ]),
                )
              else
                Container(
                      ...
                      return Column(children: [
                        ListTile(
                          ...
                          leading: Container(
                            width: 50, height: 50,
                            decoration: BoxDecoration(
                              color: (tagColors[firstTag] ?? Colors.deepPurple).withValues(alpha: 0.1),
                              borderRadius: BorderRadius.circular(10),
                            ),
                            child: Icon(tagIcons[firstTag] ?? Icons.event,
                                color: tagColors[firstTag] ?? Colors.deepPurple),
                          ),
                          title: Text(e['title'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
                          subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            if ((e['date'] ?? '').isNotEmpty)
                              Text(e['date'], style: TextStyle(color: Colors.grey[600], fontSize: 12)),
