...
class _OrgFeedScreenState extends State<OrgFeedScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final profile = await _supabase.from('profiles').select().eq('id', _myId).maybeSingle();
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setModal) => Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Share Experience', style: Theme.of(ctx).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    if (captionCtrl.text.trim().isEmpty && pickedImagePath == null) return;
                    Navigator.pop(ctx);
                    String? imageUrl;
                    if (pickedImagePath != null && pickedImageName != null) {
                      final bytes = await File(pickedImagePath!).readAsBytes();
                      final path = 'posts/$_myId/${DateTime.now().millisecondsSinceEpoch}_$pickedImageName';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Feed', style: GoogleFonts.playfairDisplay(fontWeight: FontWeight.bold, fontSize: 24, color: primary)),
        actions: [
          IconButton(
              ...
              child: _posts.isEmpty
                  ? Center(
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        const SizedBox(height: 12),
                        Text('No posts yet', style: TextStyle(color: Colors.grey[600], fontSize: 16)),
                        const SizedBox(height: 8),
                        Text('Share your event experiences with volunteers!', style: TextStyle(color: Colors.grey[400], fontSize: 13), textAlign: TextAlign.center),
                        const SizedBox(height: 20),
                        ElevatedButton.icon(
                          onPressed: _showCreatePost,
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Padding(
                                ...
                                child: Row(children: [
                                  CircleAvatar(
                                    ...
                                    backgroundImage: postAvatar != null ? NetworkImage(postAvatar) : null,
                                    child: postAvatar == null
                                        ? Text(orgName.isNotEmpty ? orgName[0].toUpperCase() : 'O',
                                            style: TextStyle(fontWeight: FontWeight.bold, color: primary))
                                        : null,
                                  ),
                                  const SizedBox(width: 10),
                                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    Text(post['org_name'] ?? orgName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                                    Text(_timeAgo(post['created_at']), style: TextStyle(fontSize: 12, color: Colors.grey[500])),
                                  ])),
                                  IconButton(
                                    ...
                                    onPressed: () => showModalBottomSheet(
                                      ...
                                      builder: (_) => SafeArea(
                                        child: ListTile(
                                          ...
                                          onTap: () { Navigator.pop(context); _deletePost(post['id']); },
                                        ),
                                      ),
                                    ),
                                  ),
                              ...
                              if (post['image_url'] != null)
                                Image.network(post['image_url'], width: double.infinity, height: 300, fit: BoxFit.cover),
                              // Caption
                              if ((post['caption'] ?? '').isNotEmpty)
                                Padding(
                                  ...
                                  child: RichText(
                                    text: TextSpan(
                                      children: [
                                        TextSpan(text: '${post['org_name'] ?? orgName}  ', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black87, fontSize: 14)),
                                        TextSpan(text: post['caption'], style: const TextStyle(color: Colors.black87, fontSize: 14)),
                                      ],
                                    ),
                                  ),
                                ),
